import { z } from "zod";
import { PublicContractBaseSchema, BuilderTemplateIdSchema, BuilderToolDefinitionSchema, BuilderInputSourceSchema, BuilderInputSourceOptionSchema, BuilderCatalogRequestTipsSchema, BuilderServiceCatalogSchema, BuilderSessionBoundServiceActionNameSchema, BuilderActionNameSchema, StableIdSchema, BuilderAssetEditCatalogEntrySchema, addDuplicateBuilderInputSourceIssues } from "./base.js";
...
export const BuilderCatalogSchema = z.lazy(() => PublicContractBaseSchema.extend({
...
function addDuplicateBuilderActionIssues(context, actionNames, path) {
    const seen = new Set();
...
function addDuplicateBuilderTemplateIssues(context, templateIds, path) {
    const seen = new Set();
    const duplicates = new Set();
...
function addDuplicateBuilderAssetThemeIssues(context, themes, path) {
    const seen = new Set();
    const duplicates = new Set();
...
function addDuplicateBuilderAssetFolderIssues(context, folders, path) {
    const seen = new Set();
    const duplicates = new Set();
...
function addDuplicateBuilderAssetAliasIssues(context, themes, path) {
...
function normalizedCatalogToken(value) {
    return value.trim().toLowerCase();
}
function sameStringArray(left, right) {
    return left.length === right.length && left.every((entry, index) => entry === right[index]);
}
function addDuplicateCatalogTextIssues(context, entries, path, label) {
    const seen = new Set();
    const duplicates = new Set();
...
function addDuplicateSessionBoundActionIssues(context, actionNames, path) {
    const seen = new Set();
    const duplicates = new Set();
    for (const actionName of actionNames) {
        if (seen.has(actionName)) {
...
//# sourceMappingURL=builder-catalog.js.map