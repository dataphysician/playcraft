...
export const ID_CONSTRAINT: RegistryConstraint = {
  kind: "ids",
...
export const VERSION_CONSTRAINT: RegistryConstraint = {
  kind: "version",
  evaluate(entry, query) {
...
export const CAPABILITY_TAGS_CONSTRAINT: RegistryConstraint = {
...
export const RENDER_CAPABILITY_CONSTRAINT: RegistryConstraint = {
...
export const DOMAIN_PROFILE_CONSTRAINT: RegistryConstraint = {
  kind: "domain-profile",
  evaluate(entry, query) {
    if (!query.domainProfileId) return [];
...
export const SAFETY_POLICY_CONSTRAINT: RegistryConstraint = {
  kind: "safety-policy",
  evaluate(entry, query) {
    if (!query.safetyPolicyId) return [];
...
export const AGE_BAND_CONSTRAINT: RegistryConstraint = {
  kind: "age-band",
  evaluate(entry, query) {
    if (!query.ageBand) return [];
...
export const MODALITY_CONSTRAINT: RegistryConstraint = {
  kind: "modality",
  evaluate(entry, query) {
    if (!query.modality) return [];
...
export const MECHANIC_IDS_CONSTRAINT: RegistryConstraint = {
  kind: "mechanic-ids",
  evaluate(entry, query) {
    if (!query.mechanicIds) return [];
    const supportedMechanics = getStringArray(entry, "supportedMechanicIds");
    if (supportedMechanics.length === 0) return [];
...
export const RULE_CATEGORY_CONSTRAINT: RegistryConstraint = {
  kind: "rule-category",
  evaluate(entry, query) {
...
export const CONTENT_TYPE_CONSTRAINT: RegistryConstraint = {
  kind: "content-type",
  evaluate(entry, query) {
...
export const FORMAT_CONSTRAINT: RegistryConstraint = {
  kind: "format",
  evaluate(entry, query) {
...
export const OFFLINE_ONLY_CONSTRAINT: RegistryConstraint = {
  kind: "offline-only",
  evaluate(entry, query) {
...
export const CREDENTIALS_FORBIDDEN_CONSTRAINT: RegistryConstraint = {
...
export const SEED_SUPPORT_REQUIRED_CONSTRAINT: RegistryConstraint = {
...
export const DEFAULT_REGISTRY_CONSTRAINTS: RegistryConstraint[] = [
  ID_CONSTRAINT,
  VERSION_CONSTRAINT,
