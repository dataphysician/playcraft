...
The current user-facing app path is `apps/studio` for the web studio and `apps/mobile-shell` for a Tauri Mobile-facing shell. Both route assembly through the `@playcraft/service` transport, which can later be replaced by a server-backed adapter as long as it preserves `BuilderServiceRequest` and `BuilderServiceResponse`. The service package includes an in-process transport, HTTP JSON client/server-body helpers, and a local `playcraft-service-http` server over the same envelope. Agents can also use the `playcraft-service` CLI bin for catalog, assemble, update, preview, reset, or raw `BuilderServiceRequest` commands over the same validated service boundary. Template switching is driven by catalog `requestAliases`, not app-specific game-name branches.
