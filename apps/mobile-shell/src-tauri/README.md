...
- Game assembly, update, preview, catalog, profile export, and profile import actions are provided by local Playcraft builder tools.
...
- Local transcript adapters can pass validated `MoonshineTranscriptRecord` objects through the Studio client; the shell does not need native microphone permissions for the current text/transcript path.
