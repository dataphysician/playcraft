...
describe("studio UI", () => {
      ...
      components: profileA.components.map((component) =>
        component.renderCapability === "component:reveal-card-grid"
          ? {
              ...
              props: {
                ...component.props,
                cards: ["moon", { label: "json-card" }, "drum"],
                pairs: {
                  moon: "pair-one",
                  drum: "pair-one"
                }
  ...
  it("plays the sequence profile through completion", async () => {
    ...
    expect(await screen.findByText("Not blue. Try green next.")).toBeDefined();
  ...
  it("fails closed when a selected trusted preview component key has duplicate component bindings", () => {
